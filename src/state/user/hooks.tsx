...
export function useAudioModeManager(): [boolean, () => void] {
  ...
  const toggleSetAudioMode = useCallback(() => {
    if (audioPlay) {
      dispatch<any>(muteAudio())
    } else {
      dispatch<any>(unmuteAudio())
    }
  }, [audioPlay, dispatch])
...
export function useAddUserToken(): (token: Token) => void {
  ...
  return useCallback(
    (token: Token) => {
      dispatch<any>(addSerializedToken({ serializedToken: serializeToken(token) }))
    },
    [dispatch]
  )
}
...
export function useRemoveUserAddedToken(): (chainId: number, address: string) => void {
  ...
  return useCallback(
    (chainId: number, address: string) => {
      dispatch<any>(removeSerializedToken({ chainId, address }))
    },
    [dispatch]
  )
}
...
export function usePairAdder(): (pair: Pair) => void {
  ...
  return useCallback(
    (pair: Pair) => {
      dispatch<any>(addSerializedPair({ serializedPair: serializePair(pair) }))
    },
    [dispatch]
  )
}
...
export function useTrackedTokenPairs(): [Token, Token][] {
  ...
  return useMemo(() => {
    ...
    const keyed = combinedList.reduce<{ [key: string]: [Token, Token] }>((memo, [tokenA, tokenB]) => {
      ...
      const _memo = memo;
      if (_memo[key]) return _memo
      _memo[key] = sorted ? [tokenA, tokenB] : [tokenB, tokenA]
      return _memo
    }, {})
